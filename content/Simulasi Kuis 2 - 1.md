...
|4.1|Sebuah algoritma _eager learner_ yang membangun model secara top-down dan greedy.|||||||||
...
- **30. (4Skenario Analisis:** (a) Untuk skenario **Filter Spam**, metrik mana (Presisi atau Recall) yang lebih penting? (b) Untuk skenario **Deteksi Penipuan (Fraud)**, metrik mana yang lebih penting? Berikan justifikasi singkat untuk keduanya.
...
- **32. (8 poin) Gain Ratio (Riwayat_Kredit):** Anda akan mengevaluasi atribut `Riwayat_Kredit`.
    ...
    - _PENTING:_ Sesuai batasan, perlakukan _missing value_ ('?') sebagai kategori keempat yang terpisah.
...
- **38. (3 poin)** Untuk cabang `Riwayat_Kredit = 'Baik'`, data tersisa adalah [D1(Ya), D2(Ya), D6(Ya)]. Cabang ini tidak murni (data: [2Y, 1T]). Oops, data di deskripsi Soal 32 salah. Mari kita gunakan data aktual: D1(Ya), D2(Ya), D6(Ya).
    
    - _Koreksi Data Aktual:_
    ...
    - Data untuk cabang 'Baik' adalah 3 data [3 Ya, 0 Tdk]. Apa yang terjadi pada cabang ini?
...
- **32. (8 poin)**
    ...
    - (a) Information Gain:
        ...
        = $E(S) - [ (3/10)E_{Baik} + (2/10)E_{Buruk} + (3/10)E_{Cukup} + (2/10)E_{?} ]$
        
        = $0.971 - [ (0.3 \times 0.918) + (0.2 \times 0) + (0.3 \times 0.918) + (0.2 \times 1.0) ]$
        
        = $0.971 - [ 0.2754 + 0 + 0.2754 + 0.2 ]$
        
        = $0.971 - 0.7508 = \textbf{0.2202}$
    ...
    - (c) **Gain Ratio:** = Gain / SplitInfo = $0.2202 / 1.972 = \textbf{0.1117}$  
